...
- Every form's submit button disables on three conditions, OR'd together, each included only when it applies to that form: `f().invalid()` (never submit a well-formed-but-invalid form — always include this), `f().submitting()` (never allow a double-submit while a request is in flight — always include this; note this is separate from `[btnLoading]="f().submitting()"`, which only drives the spinner and doesn't itself block clicks), and — only on forms that can edit an existing record — `(isEditing() && !f().dirty())` (nothing to save if nothing changed; `isEditing` is a `computed()` signal on most dual-purpose forms, called as `isEditing()`, but a plain boolean on a few — call it bare, no parens, where that's the case). Pure-create/one-shot-action forms omit the third clause entirely — there's no "unchanged" state worth blocking on when the model started empty. See `organization-settings.component.ts` (edit-only: `[disabled]="f().invalid() || f().submitting() || !f().dirty()"`) and `employee-form-dialog.component.ts` (dual-purpose with a boolean `isEditing`: `[disabled]="f().invalid() || f().submitting() || (isEditing && !f().dirty())"`) for reference implementations. If `dirty()` alone ever proves too coarse for a specific form (e.g. it doesn't reset after the user edits a value back to its original), switch that form to a real deep equality check against the initial model instead — don't add one everywhere preemptively.
