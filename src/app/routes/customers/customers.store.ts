...
const DEFAULT_ERROR_MESSAGE = 'Não foi possível carregar os clientes.';
...
const initialMeta: CustomersMeta = {
  total: 0,
  page: 1,
  name: '',
  reloadTrigger: 0,
  ...
  withMethods((store, customerService = inject(CustomerService)) => ({
    load: rxMethod<LoadParams>(
      pipe(
        ...
        switchMap(({ name, page }) =>
          customerService.list({ page, limit: PAGE_SIZE, name: name || undefined }).pipe(
            tapResponse({
              next: (result) => {
              ...
              error: (error: unknown) => {
                patchState(store, setAllEntities<Customer>([]));
                patchState(store, {
                  total: 0,
                  page,
    ...
    createCustomer(payload: CustomerPayload) {
      return customerService.create(payload).pipe(
        tap((customer) => {
          patchState(store, prependEntity({ id: customer.id, name: customer.name }));
    ...
    deleteCustomer(customer: Customer) {
      ...
      return customerService.delete(customer.id).pipe(
        tapResponse({
          next: () => {
